...
is_some_order_cancelled = handle_placed_orders(cur_orders, orders_relevancy, highest_bid_price, lowest_ask_price)
...
for order_type in ("bid", "ask"):
    ...
    if is_above_min_size(pair, amount):
        logging.info("Placing {} order, price: {}, amount: {}".format(order_type, price, amount))

        order_id = create_order(pair, amount, price)['info']
        cur_orders[order_type].add(order_id)


def handle_placed_orders(cur_orders: Dict[str, Orders], orders_relevancy
